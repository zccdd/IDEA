...
<div data-options="region:'center',title:'欢迎来到坤哥Java开发软件'" style="padding:5px;background:#eee;">
    ...
    <table id="teaTable2321" style="height: 500px"></table>
</div>
<script type="text/javascript">
    ...
    function search(cknodes2){
        ...
        //alert($("#d43113").val());
        $('#teaTable2321').datagrid({
            url:'<%=request.getContextPath()%>/goods/queryshenGoods.do',
            pagination:true,
            pageNumber:1,
            pageSize:3,
            pageList:[3,5,8,10],
...
//					var arrs = $('#dg').datagrid('getSelections');
                    var arrs = $('#teaTable2321').datagrid('getChecked');
                    var gwpid="";
            ...
            columns:[[
                ...
                {field: 'str', title: '操作', width: 125, align: 'right',
                    formatter: function (value, row, index) {
                        ...
                        return "<input type='button' value='驳回' onclick='bo("+row.gid+")'/><input type='button' value='通过' onclick='tong("+row.gid+")'/>"
                    },
                }
            ]]
        });
    ...
    function bo(gid){
        $.ajax({
            url:"<%=request.getContextPath()%>/goods/updatebo.do",
            type:"post",
            data:"gid="+gid,
            success:function(da){
                $.messager.show({
    ...
    function tong(gid){
        $.ajax({
            url:"<%=request.getContextPath()%>/goods/updatetong.do",
            type:"post",
            data:"gid="+gid,
            success:function(da){
                $.messager.show({
